...
async function clearLogs(res, userId) {
  try {
    const nooftransactions = await Transaction.find({ userId: userId }).countDocuments();
    if (nooftransactions > 20) {
...
export const addPurchasedStock = async (req, res) => {
  try {
    ...
    const alreadyPurchased = await PurchasedStock.find({ userId: req.userId, stock: stock._id }).countDocuments() > 0;
    if (alreadyPurchased) {
    ...
    const cost = sharesBought * stock.currentPrice;
    if (cost > user.coins) {
    ...
    const remcoins = user.coins - cost;

    await User.findByIdAndUpdate(req.userId, { coins: remcoins });
...
export const updatePurchasedStock = async (req, res) => {
  ...
  try {
    ...
    if (bought < 0) {
      ...
      initialInvestment: purchased.initialInvestment - (Math.abs(bought) * stock.currentPrice) });
